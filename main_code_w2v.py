...
if not os.path.exists(w2v_save_path):
  model = Word2Vec(sentences, size=256, window=4, min_count=1, workers=2, sg=1)
...
embedder = Embedding(vocab_size + 1, 256, input_length = 600, weights=[embedding_matrix], trainable=False)
# embedder = Embedding(vocab_size + 1, 256, input_length = 600, weights=[embedding_matrix])
...
model.save(os.path.join(save_dir,'w2v_model.h5'))
